...
const RecipeFilter: React.FC<FiltersComponentProps> = ({ onApplyFilters }) => {
  const [filters, setFilters] = React.useState<RecipeFilters>(initialFiltersState);
  ...
  const toggleFilter = (category: keyof RecipeFilters, filter: string) => {
    setFilters(prevFilters => {
      ...
      if (filterIndex === -1) {
        ...
        return { ...prevFilters, [category]: currentFilters.filter(f => f !== filter) };
      }
    });
  };
  ...
  return (
    <div className="filters-container">
      ...
      <div className="filters-section">
        <h3>Diets</h3>
        {['balanced', 'high-fiber', 'high-protein', 'low-carb', 'low-fat', 'low-sodium'].map((filter) => (
          <div className="filter-option" key={filter}>
            <label>
              {filter.replace(/-/g, ' ')}
              <input
      ...
      <button  style={{ width: "150px" }}className="search-button" onClick={handleApplyFilters}>SEARCH</button>
    </div>
  );
};
